...
function isAllowedLiteral(text) {
  ...
  if (/^[A-Z_]+$/.test(t)) return true; // enum
  if (/^\/[a-z/]+$/.test(t)) return true; // route
  if (ALLOW_PATTERNS.some((re) => re.test(t))) return true;
  return false;
}

function scanFile(file) {
  if (IGNORE_FILES.some((re) => re.test(file))) return [];
  ...
  while ((m = jsxTextRe.exec(content))) {
    ...
    if (line.includes('t(') || line.includes('{t') || line.includes('<Trans')) continue;
  ...
  while ((m = ariaRe.exec(content))) {
    ...
    if (literal.includes('.') && literal.split('.').length > 1 && /^[a-z.]+$/.test(literal.toLowerCase())) continue;
...
md += `**Generated:** ${new Date().toISOString()} **SHA:** ${process.env.GITHUB_SHA ?? 'local'} **Files scanned:** ${files.length} **Hits:** ${allHits.length}\n\n`;
