...
describe('capabilities', () => {
    ...
    it('NAVIGATION entries all reference a known capability', () => {
        ...
        for (const entry of NAVIGATION) {
            if (entry.capability) {
                expect(
                    all.has(entry.capability),
                    `${entry.title} capability ${entry.capability} must be in ALL`,
                ).toBe(true);
            }
        }
    });
    ...
    it('NAVIGATION workspace values are valid', () => {
        for (const entry of NAVIGATION) {
            expect(
                ['admin', 'agent', 'settings'].includes(entry.workspace),
            ).toBe(true);
        }
    });
    ...
    it('domain visibility: Agent sees only Agent', () => {
        ...
        const visible = FLEX_DOMAINS.filter((d) => has(d.capability)).map(
            (d) => d.id,
        );
    ...
    it('domain visibility: Super Admin sees all', () => {
        ...
        expect(visible.sort()).toEqual(
            ['administration', 'agent', 'platform', 'supervision'].sort(),
        );
    });

    it('Platform domain gated by tenants.manage (not roles.manage)', () => {
        expect(FLEX_DOMAINS.find((d) => d.id === 'platform')?.capability).toBe(
            'tenants.manage',
        );
    });

    it('Management Console module gating', () => {
        const hasSupervisor = (c: string) =>
            (ROLE_CAPABILITIES.supervisor as string[]).includes(c);
        const hasAdmin = (c: string) =>
            (ROLE_CAPABILITIES.admin as string[]).includes(c);
        const hasSuper = (c: string) =>
            (ROLE_CAPABILITIES['super-admin'] as string[]).includes(c);
        const visibleFor = (has: (c: string) => boolean) =>
            CONSOLE_MODULES.filter(
                (m) => !m.capability || has(m.capability),
            ).map((m) => m.id);
