...
const headers: { label: string; align: 'start' | 'end' | 'center' }[] = [
    { label: 'Report Name', align: 'start' },
    { label: 'Schedule', align: 'start' },
    { label: 'Target', align: 'start' },
    { label: 'Format', align: 'start' },
    { label: 'Last Run', align: 'start' },
    { label: 'Next Run', align: 'start' },
                        ...
                        {records.map((schedule) => {
                            ...
                            return (
                                <tr key={schedule.id} className="border-b border-border last:border-b-0 hover:bg-muted/30 transition-colors">
                                    ...
                                    <td className="px-4 py-2.5 text-xs text-flex-text-primary whitespace-nowrap text-start">{schedule.scheduleSummary}</td>
                                    <td className="px-4 py-2.5 text-xs text-flex-text-primary whitespace-nowrap text-start">{schedule.targetSummary}</td>
                                    <td className="px-4 py-2.5 text-xs font-semibold text-flex-text-primary whitespace-nowrap text-start">{schedule.format}</td>
                                    <td className="px-4 py-2.5 text-xs tabular-nums text-flex-text-muted whitespace-nowrap text-start">{schedule.lastRun}</td>
                                    <td className="px-4 py-2.5 text-xs tabular-nums text-flex-text-muted whitespace-nowrap text-start">{schedule.nextRun}</td>
                                    <td className="px-4 py-2.5 text-start"><ExecutionStateBadge state={schedule.executionState} /></td>
                                    <td className="px-4 py-2.5 text-start"><ScheduleStatusBadge status={schedule.status} /></td>
