...
function DataGridColumnHeaderInner<TData extends object, TValue>({
  ...
  const sortIcon =
    canSort &&
    (isSorted === "desc" ? (
      <RiArrowDownLine className="size-3.25" aria-hidden="true" />
    ) : isSorted === "asc" ? (
      <RiArrowUpLine className="size-3.25" aria-hidden="true" />
    ) : (
      <RiExpandUpDownLine className="mt-px size-3.25" aria-hidden="true" />
    ))
  ...
  const menuItems = useMemo(() => {
    ...
    if (canSort) {
      ...
      items.push(
        ...
        >
          ...
          {isSorted === "asc" && (
            <RiCheckLine className="text-primary size-4 opacity-100!" />
          )}
        </DropdownMenuItem>,
        <DropdownMenuItem
          key="sort-desc"
    ...
    if (props.tableLayout?.columnsPinnable && canPin) {
      ...
      items.push(
        ...
        >
          <RiContractLeftLine className="size-3.5!" aria-hidden="true" />
    ...
    return (
      <div className="-ms-2 flex h-full items-center justify-between gap-1.5">
        ...
        {props.tableLayout?.columnsPinnable && canPin && isPinned && (
          ...
          >
            <RiUnpinLine className="size-3.5! opacity-50!" aria-hidden="true" />
          </Button>
        )}
      </div>
    )
