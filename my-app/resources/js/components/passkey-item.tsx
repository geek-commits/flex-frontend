        ...
        <div className="flex items-center justify-between border-b p-4 last:border-b-0">
            <div className="flex items-center gap-4">
                ...
                <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                        {t('settings.passkeys.added', { date: passkey.created_at_diff })}
                        {passkey.last_used_at_diff && (
                            <>
                                <span className="mx-1 text-muted-foreground/50">/</span> {t('settings.passkeys.lastUsed', { date: passkey.last_used_at_diff })}
                            </>
                        )}
                    </p>
                </div>
            ...
            <Dialog>
                ...
                <DialogContent>
                    ...
                    <DialogDescription>
                        {t('settings.passkeys.removeDescription', { name: passkey.name })}
                    </DialogDescription>
                    <DialogFooter className="gap-2">
                        <DialogClose render={<Button variant="secondary">{t('settings.passkeys.cancel')}</Button>} />
                        <Button variant="destructive" onClick={handleDelete} disabled={isDeleting}>
                            {isDeleting ? t('settings.passkeys.removing') : t('settings.passkeys.removeConfirm')}
                        </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
